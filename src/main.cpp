...
public:
    ...
    void remove_subscriber_track(rtc::Track* track) noexcept {
        std::lock_guard lock(mutex);
...
struct PublisherInfo {
    ...
    alignas(32) std::atomic<size_t> publisher_count{0};
    ...
    add_publisher(std::string_view publisher_id, std::string_view stream_type) {
        ...
        try {
            ...
            size_t publisher_index{0};
            {
                ...
                publishers.push_back(std::make_unique<PublisherInfo>(std::move(pc), std::string(publisher_id), std::string(stream_type)));
            }
            ...
            publisher->pc->onStateChange([this, publisher_index](rtc::PeerConnection::State state) {
                ...
                if (state == rtc::PeerConnection::State::Disconnected ||
                  ...
                  //  cleanupPublisher(publisherIndex);
                }
            });
